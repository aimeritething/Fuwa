...
import { isDocumentPath, noteEntryForPath } from '../utils/noteEntry'
 ...
 * directory as the root outside the Folder, or the Folder inside it (ADR-0002). `setTabs` keeps the save hook's contract: it may replace a Tab's
...
export function useNoteTabs(folder?: string | null) {
  ...
  const restoreOpenEditors = useCallback(async (editors: SessionEditor[], activePath: string | null, restoredFolder?: string | null) => {
    ...
    const survivors = await readSurvivingTabs(documents, restoredFolder)
