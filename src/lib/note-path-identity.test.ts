...
describe('note-path-identity', () => {
  ...
  it('joins vault paths without changing Windows verbatim roots', () => {
    const vaultPath = String.raw`\\?\C:\Users\alex\Notes`
    expect(joinVaultPath(vaultPath, 'note.md')).toBe(String.raw`\\?\C:\Users\alex\Notes/note.md`)
  })

  it('extracts filenames from slash or backslash paths', () => {
    expect(notePathFilename(String.raw`C:\Users\alex\Notes\note.md`)).toBe('note.md')
  })
})
