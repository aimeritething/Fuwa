...
describe('PathRow', () => {
  ...
  it('fills the right-hand slot on an Image Tab with its dimensions, size and two hand-offs', () => {
    render(<PathRow filename="lake.png" image={imageSlot} />)
  ...
  it('keeps the metadata slot empty until the picture has loaded, buttons and all in place', () => {
    render(<PathRow filename="lake.png" image={{ ...imageSlot, metadata: null }} />)
  ...
  it('hands the file over from either button', () => {
    ...
    render(<PathRow filename="lake.png" image={{ metadata: null, onOpenExternal, onCopyPath }} />)
  ...
  describe('the Rich | Raw control', () => {
    ...
    it('shows the Frontmatter badge before the control, and switches to Raw on click', () => {
      const onChange = vi.fn()
      renderWithTooltips(
        <PathRow filename="Fuwa.md" mode={richMode({ onChange, frontmatterLabel: 'frontmatter · 2 keys' })} />,
      )
      ...
      expect(order).toEqual(['path-row-frontmatter', 'path-row-mode'])
