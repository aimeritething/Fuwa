...
} from './ime-composition-key-guard-extension'
...
describe('createSafariImeDomPreserverPlugin', () => {
  it('keeps an out-of-model sentinel beside composing text until Safari finishes the commit', () => {
    ...
    const sentinel = container.querySelector('[data-plumo-ime-dom-preserver]')
    ...
    expect(container.querySelector('[data-plumo-ime-dom-preserver]')).toBeNull()
    expect(view.state.doc.textContent).toBe("ce'shi")
    view.destroy()
  })
})
