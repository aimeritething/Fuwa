...
- **Byte size**: the listing's `fileSize`, which the spec names as the source, formatted to one decimal.
...
- An Image file can only reach a Tab from inside the Folder, which is what the spec says for v0.1 (no Finder open, no drop, no dialog). An Image file outside the Folder would have no listing behind it and would show a size of `0 B`.
