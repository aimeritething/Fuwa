...
- The Pinned lists are the renderer's as well: `pinned` maps each Folder's path to its pinned paths in the user's order, and every list rides along in every update, not only the open Folder's, so opening a Folder again gets its pins back. The sidebar's `collapsedSections` (today only `"pinned"`) is the sections folded away under their label. A pin whose file the Folder's listing no longer holds is dropped by the renderer when that Folder is listed; the Rust side still treats all of it as opaque JSON.
