...
- The theme moved into the renderer's part of the Session with AIM-382: the file's `theme` is the View → Appearance choice, restored at launch and written on every change. Tolaria's localStorage key stays only as a pre-paint mirror for the inline script in `index.html`, so the first frame already has the right appearance; the theme applier rewrites it whenever the choice changes. Sidebar state follows with the sidebar ticket.
