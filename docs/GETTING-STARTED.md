...
The global Quick Launcher uses a native OS registration rather than the focused-window command registry. Its default is `Cmd+Option+T` on macOS and `Ctrl+Alt+T` elsewhere; Settings records a replacement chord and reports registration conflicts while preserving the last working shortcut. The shortcut focuses one singleton `quick-launcher` webview even when the main window is hidden. Its single Cmd+O-style input searches every opted-in registered vault. When the input has no matches, the launcher offers to create it as a new note and exposes only the destination-vault control at the bottom.
